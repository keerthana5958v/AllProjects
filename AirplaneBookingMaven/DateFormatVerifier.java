...
public class DateFormatVerifier {
    ...
    int thisMonth = Integer.parseInt(LocalDate.now().toString().substring(5, 7));
    int thisDate = Integer.parseInt(LocalDate.now().toString().substring(8, 10));
    ...
    static int getDateCount = 0;

    public String input() {
        Scanner scanObj = new Scanner(System.in);
        if (getDateCount == 0) {
            System.out.println("Enter the date in DD/MM/YYYY format:");
        } else if (getDateCount <= 5) {
            System.out.println(ConsoleColors.YELLOW + "Please select your flight date starting from tomorrow in DD/MM/YYYY format\nDate must not exceed 2 months from today" + ConsoleColors.RESET);
        } else {
            System.out.println(ConsoleColors.RED + "Please select your flight date starting from tomorrow in DD/MM/YYYY format\nDate must not exceed 2 months from today" + ConsoleColors.RESET);
        }
    ...
    public boolean isValidMonth() {
        if (getYear() > thisYear) {
            ...
            return ((getMonth() >= 1 && getMonth() <= thisMonth) && (getYear() == thisYear));
        }
//        return false;
    }
