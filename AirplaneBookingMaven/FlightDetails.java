...
public class FlightDetails {
    ...
    static int getAirlineCount = 0;

    String getAirline() {
        Scanner sc = new Scanner(System.in);
        if (getAirlineCount == 0) {
            System.out.println("Select your Airline by number: " + ConsoleColors.RESET);
        } else if (getAirlineCount < 5) {
            System.out.println(ConsoleColors.YELLOW + "Select your Airline by number: " + ConsoleColors.RESET);
        } else {
            System.out.println(ConsoleColors.RED + "PRESS your Airline number (1-5): " + ConsoleColors.RESET);
        }
        getAirlineCount++;
        return sc.nextLine();
    }
    ...
    static int getFlightClassCount = 0;
    ...
    public String returnFlightClass() {
        ...
        do {
            if (getFlightClassCount == 0) {
                System.out.println("Enter " + ConsoleColors.CYAN_BOLD + "1" + ConsoleColors.RESET + " for Economy, " + ConsoleColors.CYAN_BOLD + "2" + ConsoleColors.RESET + " for Business, " + ConsoleColors.CYAN_BOLD + "3" + ConsoleColors.RESET + " for First Class");
            } else if (getFlightClassCount < 5) {
                System.out.println(ConsoleColors.YELLOW + "Enter 1 for Economy, 2 for Business, 3 for First Class" + ConsoleColors.RESET);
            } else {
                System.out.println(ConsoleColors.RED + "Enter 1 for Economy, 2 for Business, 3 for First Class" + ConsoleColors.RESET);
            }
            getFlightClassCount++;
            input = scanner.nextLine();
            result = selectFlightClass(input);
            if (result.equals("F")) {
                System.out.println(ConsoleColors.GREEN + "You have selected First class ticket" + ConsoleColors.RESET);
            }
            if (result.equals("B")) {
                System.out.println(ConsoleColors.GREEN + "You have selected Business class ticket" + ConsoleColors.RESET);
            }
            if (result.equals("E")) {
                System.out.println(ConsoleColors.GREEN + "You have selected Economy class ticket" + ConsoleColors.RESET);
            }
    ...
    String seat = ConsoleColors.YELLOW_BACKGROUND_BRIGHT + "   " + ConsoleColors.RESET + "  ";
    ...
    public int changeBookedSeat() {
        int seatNum = Integer.parseInt(checkSeatValidity());
        int index = (seatNum - 1) * 17;
        String newSeat = String.valueOf(this.seat).repeat(12);

        newSeat = newSeat.substring(0, index) + this.bookedSeat + newSeat.substring(index, (newSeat.length() - 17));
    ...
    public String checkSeatValidity() {
        String seatNum;
        do {
            seatNum = getSeatNum();
    ...
    public boolean isValidSeatNumber(String seatNum) {
        try {
    ...
    String getLocation() {
        ...
        if (getLocationCount == 0) {
            ...
            System.out.println(ConsoleColors.RED + "Enter your location by number:" + ConsoleColors.RESET);
        }
        getLocationCount++;
        return sc.nextLine();
    }
    ...
    static int getDestinationCount = 0;
