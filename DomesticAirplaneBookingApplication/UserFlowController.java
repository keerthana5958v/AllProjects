...
public class UserFlowController {
    protected void welcomeUser(){
        printSlowly(ConsoleColors.WHITE_BOLD_BRIGHT+"Welcome to our domestic airline booking application! \nWe're thrilled to have you on board as we embark on a journey of convenience and adventure"+ConsoleColors.RESET,20);
    }
    protected void askFAQ() throws InterruptedException {
    ...
    private boolean isProperFAQInput(String input){
    ...
    private void displayFAQQuestion(){
    ...
    protected void thankYouUser() throws InterruptedException {
    ...
    protected void completePayment(){
        printSlowly(ConsoleColors.GREEN_BOLD_BRIGHT+"_".repeat(30), 300);
        System.out.println("\nPayment completed"+ConsoleColors.RESET);

    }
    protected void displaySeatInfo(){
    ...
    protected void printSlowly(String str, int millis){
        for(int i = 0 ; i < str.length() ; i++){
            System.out.print(str.charAt(i));
            try{
            Thread.sleep(millis);}catch(Exception ignored){}
        }
    }
